...
function handleGatewayPayment(gateway, order, amount, customer) {
  switch(gateway) {
    ...
    case 'sabpaisa':
      ...
      break;
    case 'instamojo':
      handleInstamojoPayment(order);
      break;
    case 'easebuzz':
      handleEasebuzzPayment(order);
      break;
  }
}
...
function handleInstamojoPayment(order) {

  if (!order.redirectUrl) {
    showPaymentStatus(
      'error',
      'Payment Gateway Error',
      'Instamojo redirect URL missing'
    );
    return;
  }
...
function handleEasebuzzPayment(order) {

  if (!order.redirectUrl) {
    showPaymentStatus(
      'error',
      'Payment Gateway Error',
      'Easebuzz redirect URL missing'
    );
    return;
  }
